...
class MessageSenderIntegrationTests: XCTestCase {
    ...
    private func createMockMessage(attachmentHash: String?) -> MockMessage {
        let message = MockMessage(messageId: UUID().uuidString)
        if let hash = attachmentHash {
            let attachment = MockAttachment(uniqueId: UUID().uuidString, contentType: "image/jpeg")
            attachment.mockHashString = hash
...
class MockMessage {
    ...
    init(messageId: String, attachments: [MockAttachment]? = nil) {
        self.messageId = messageId
        self.mockAttachments = attachments
    }
}
...
class MockGlobalSignatureService {
    private(set) var checkedHashes = Set<String>()
    private(set) var storedHashes = Set<String>()
...
class MockMessageSender {
    ...
    func trySendingMessage(_ message: MockMessage) async -> (success: Bool, error: Error?) {
        // Check local blocklist first
        if let hash = message.mockAttachments?.first?.mockHashString,
           mockDuplicateStore.isBlocked(hash) {
            return (false, MessageSenderError.duplicateBlocked(hash: hash))
        }
        
        // Then check global database
        if let hash = message.mockAttachments?.first?.mockHashString,
           mockSignatureService.contains(hash) {
            return (false, MessageSenderError.duplicateBlocked(hash: hash))
        }
        
        if shouldFailSend {
            return (false, MessageSenderError.networkError)
        }
        
        sendAttemptCount += 1
        
        // Store hash on successful send
        if let hash = message.mockAttachments?.first?.mockHashString {
            _ = mockSignatureService.store(hash)
        }
        
        return (true, nil)
    }
    
    func trySendingMessageWithRetry(_ message: MockMessage, maxRetries: Int) async -> (success: Bool, error: Error?) {
        var retryCount = 0
        while retryCount <= maxRetries {
            let result = await trySendingMessage(message)
            if result.success || result.error is MessageSenderError {
                return result
            }
            retryCount += 1
            try? await Task.sleep(nanoseconds: UInt64(pow(2.0, Double(retryCount)) * 1_000_000_000))
        }
        return (false, MessageSenderError.maxRetriesExceeded)
    }
}
