...
import WebRTC
import AWSCore
import DuplicateContentDetection // Add this import
...
final class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    private func setAppIsReady(
        ...
        appReadiness.runNowOrWhenMainAppDidBecomeReadyAsync {
            Task {
                Logger.info("[AWS Init] Starting AWS initialization and validation for Duplicate Content Detection...")
                ...
                // 1. Ensure AWS Credentials are configured using the correct config from the DuplicateContentDetection module
                // Note: Using the AWSConfig from DuplicateContentDetection module explicitly
                DuplicateContentDetection.AWSConfig.setupAWSCredentials()

                // 2. Verify Credentials using the Verification Manager
                var awsVerified = false
                var verificationError: Error?
                do {
                    // Verify Cognito, API Gateway, and DynamoDB table existence
                    awsVerified = try await AWSCredentialsVerificationManager.shared.verifyCredentialsAsync(
                        setupCredentialsIfNeeded: false, // Already called setup above
                        checkAPIGateway: true,
                        verifyTableExists: true
                    )
                    
                    if awsVerified {
                        Logger.info("[AWS Init] ✅ AWS credentials, API Gateway, and DynamoDB table verified successfully.")
                    } else {
                        // This path might indicate a configuration issue not throwing an error,
                        // though verifyCredentialsAsync typically throws on failure. Log just in case.
                        Logger.error("[AWS Init] ❌ AWS verification manager reported failure without throwing an error.")
                    }
                } catch {
                    Logger.error("[AWS Init] ❌ AWS verification failed with error: \(error.localizedDescription)")
                    verificationError = error // Store error for context
                }

                // 3. Install Attachment Download Hook unconditionally
                // The hook itself should internally check AWS status via GlobalSignatureService before making AWS calls.
                let pool = storage.grdbStorage.pool
                AttachmentDownloadHook.shared.install(with: pool)
                Logger.info("[AWS Init] AttachmentDownloadHook installed.")

                // 4. Final Logging based on verification status
                if awsVerified {
                     Logger.info("[AWS Init] ✅ AWS setup complete. Attachment validation system fully operational.")
                 } else {
                     Logger.warn("[AWS Init] ⚠️ AWS setup incomplete (Verification failed: \(verificationError?.localizedDescription ?? "Unknown reason")). Attachment validation system might operate in degraded mode (e.g., bypass hash checks).")
                 }
            }
        }
        appReadiness.runNowOrWhenMainAppDidBecomeReadyAsync {
